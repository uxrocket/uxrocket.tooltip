...
(function($) {
    ...
    var bindUIActions = function($el) {
        ...
        if(opts.type === "popover"){  
            ...
            if ( opts.parentSelector && opts.parentSelector != '' ) {

                $(document).on(events.mouseenter, opts.parentSelector, function(e) {
                    var dEl = $(this).find('.uxitd-tooltip-ready');
                    var _instance = dEl.data(ns.data);
                    if(dEl && dEl != '' && _instance && _instance != ''){
                        showTooltip( _instance );
                    }
                });
                
                $(document).on(events.mouseleave, opts.parentSelector, function(e) {
                    var dEl = $(this).find('.uxitd-tooltip-ready');
                    var _instance = dEl.data(ns.data);
                    if(dEl && dEl != '' && _instance && _instance != ''){
                        hideTooltip( _instance );
                ...
                $el.on(events.mouseenter, function(e) {
                    var active = $el.data(ns.dataActive);

                    if(!active || !_instance.view) {
                        showTooltip(_instance, function( activeTooltipPop ){
                            // var activeTooltipPop = $("." + ns.pop);
                            activeTooltipPop.on(events.mouseenter, function() {
                                ttipActive = true;
                            });
                            activeTooltipPop.on(events.mouseleave, function() {
                                ttipActive = false;
                                hideTooltip(_instance);
                            });
                        });
                ...
                $el.on(events.mouseleave, function(e) {
                    var active = $el.data(ns.dataActive);

                    if(active || _instance.view) {
                        setTimeout(function(){
                            if( ttipActive ) { return; }
                            hideTooltip(_instance);
                            $("." + ns.pop).off(events.mouseenter).off(events.mouseleave);
                        }, 200);
    ...
    var showTooltip = function(options, cb) {
    ...
    var createTooltip = function(options) {
        ...
        if(typeof opts.cssClass !== 'undefined' && opts.cssClass !== null) {
            replace.cssClass = opts.cssClass || options.el.data('cssclass');
        }
