...
;(function($){
    var ux, // local shorthand
        ...
        defaults = {
            ...
            template: '<div {id} class="tooltip-pop {class}"><div class="tooltip-content">{content}</div><div class="tooltip-arrow"></div></div>',
            cssClass: '',
            id: '',
            showAnimate: false,
    ...
    var Tooltip = function(el, options){
        ...
        bindUIActions($el, instance_opts);
    };

    var bindUIActions = function($el, options){

        var opts      = options.opts,
            $ttip     = options.$ttip,
            showEvent = (opts.type === "popover" ? "click" : "mouseenter mouseleave");

        $el.on(showEvent, function() {
            var active = $el.data("tooltip-active");

            if(!active || !options.view){
                showTooltip(options);
            }else{
                hideTooltip(options);
            }
        ...
        $(document).on('click', function(e){
            ...
            if(hasTooltip){
                ...
                if($ttip.hasClass("tooltip-popover")) {return;};
    ...
    var createTooltip = function(options){
        ...
        if(obj_pattern.test(temp_content)){
            ...
            var fn = temp_content.replace(function_pattern, "$1|$2").split("|");
